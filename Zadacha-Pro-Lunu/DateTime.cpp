...
// возвращает строку вида ГГГГ
std::string DateTime::getYYYY() const {
	std::stringstream ss;
	ss << year;
	return ss.str();
}

// возвращает строку вида ГГГГММДД
std::string DateTime::getYYYYMMDD() const {
	std::stringstream ss;
	ss << year;
	if (month < 10) ss << "0";
	ss << month;
...
// вывод результата в строго требуемом формате
void DateTime::printMoonResult(const std::string& rise, const std::string& culmination, const std::string& set) const {
	cout << "Дата: ";
	if (day < 10) cout << "0";
	cout << day << ".";
	if (month < 10) cout << "0";
